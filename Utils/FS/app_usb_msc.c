...
#define W25Q128_SECTOR_SIZE     4096    // W25Q128 物理扇区大小 (4KB)
#define W25Q128_PAGE_SIZE       256
#define W25Q128_CAPACITY_BYTES  (16 * 1024 * 1024) // W25Q128 容量 (16MB)
#define W25Q128_SECTOR_COUNT    (W25Q128_CAPACITY_BYTES / W25Q128_SECTOR_SIZE) // 扇区总数

void usbd_msc_get_cap(uint8_t busid, uint8_t lun, uint32_t *block_num, uint32_t *block_size)
{
    *block_num = W25Q128_SECTOR_COUNT; //Pretend having so many buffer,not has actually.
    *block_size = W25Q128_SECTOR_SIZE;
}
int usbd_msc_sector_read(uint8_t busid, uint8_t lun, uint32_t sector, uint8_t *buffer, uint32_t length)
{
    uint32_t globalAddr = sector * FLASH_SECTOR_SIZE;
    Flash_ReadBytes(globalAddr, buffer, length);
    return 0;
}

int usbd_msc_sector_write(uint8_t busid, uint8_t lun, uint32_t sector, uint8_t *buffer, uint32_t length)
{
    uint32_t globalAddr = sector * W25Q128_SECTOR_SIZE;
    Flash_EraseSector(globalAddr);
    for (uint16_t offset = 0; offset < length; offset += W25Q128_PAGE_SIZE)
    {
        Flash_WriteInPage(globalAddr + offset, buffer + offset, W25Q128_PAGE_SIZE);
    }
    return 0;
}
#else
